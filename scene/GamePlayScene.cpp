...
void GamePlayScene::Collision()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		if (enemy[i]->GetFlag()) {
			Sphere BulletShape;
			BulletShape.center = XMLoadFloat3(&shoot->GetPosition());
			BulletShape.radius = shoot->GetScale().z;
			XMVECTOR inter;
			bool hit = Collision::CheckSphere2Sphere(BulletShape, enemyShape, &inter);
			if (hit) {
				enemy[i]->Hit();
				shoot->Hit();
			}
